...
class UnifiedScheduleTests(TestCase):
    def setUp(self):
        ...
        self.week = Week.objects.create(season=self.season, week_number=1, monday_date='2024-01-01')
    ...
    def test_unified_function_create_mode(self):
        ...
        response = save_or_update_schedule(request, season_id=None, skip_validation=True)
        ...
        from scheduler.models import OffWeek
    ...
    def test_unified_function_update_mode(self):
        ...
        factory = RequestFactory()
        request = factory.post('/test/', json.dumps(data), content_type='application/json')
        response = save_or_update_schedule(request, season_id=self.season.id, skip_validation=True)
        ...
        # Verify week date was updated
        self.week.refresh_from_db()
        self.assertEqual(str(self.week.monday_date), '2024-02-01')
    ...
    def test_unified_function_validation_errors(self):
        ...
        # Verify season was created but rolled back due to validation error
        self.assertFalse(Season.objects.filter(name='Error Test Season').exists())
